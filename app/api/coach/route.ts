export async function POST(request: Request) {
  try {
    ...
    const { japanese, translation, sourceLanguage = 'ja' } = await request.json();
    const response = await fetch('https://api.shisa.ai/openai/v1/chat/completions', {
      ...
      body: JSON.stringify({ model: 'shisa-ai/shisa-v2.1-llama3.3-70b', temperature: 0.2, stream: false, messages: [
        ...
        { role: 'user', content: sourceLanguage === 'en' ? `The worker needs to say this in Japanese: ${japanese}\nJapanese translation: ${translation}` : `Heard Japanese: ${japanese}\nEnglish meaning: ${translation}` }
      ] })
    });
